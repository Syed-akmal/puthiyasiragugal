...
class Admin extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['total_loan_amount_in_process'] = $this->db->select_sum('loan_amount')->where('loan_status', 'in_process')->get('loan')->row()->loan_amount;
        ...
        $data['total_amount_difference'] = $this->db->query("SELECT SUM(loan.loan_amount) - COALESCE(SUM(term.amount), 0) as total_difference FROM loan LEFT JOIN term ON loan.id = term.loan_id WHERE loan.loan_status = 'in_process' AND loan.id NOT IN (SELECT loan_id FROM term WHERE loan_status = 'completed')")->row()->total_difference;



        // $data['monthlyloanchart'] = $this->db->query("SELECT DATE_FORMAT(created_on, '%Y-%m') AS month, COUNT(*) AS count FROM loan GROUP BY month")->result();

        // Extracting chart data
        // $chartLabels = [];
        // $chartData = [];

        // foreach ($data['monthlyloanchart'] as $row) {
